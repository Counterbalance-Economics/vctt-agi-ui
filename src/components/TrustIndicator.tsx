...
export default function TrustIndicator({ trustScore, compact = false }: TrustIndicatorProps) {
  ...
  if (compact) {
    return (
      <div className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium ${getTrustBgColor(trustScore)} ${getTrustColor(trustScore)}`}>
        <Shield size={12} />
  ...
  return (
    <div className={`flex items-center gap-2 p-3 rounded-lg border ${getTrustBgColor(trustScore)} ${getTrustColor(trustScore)}`}>
      <Shield size={18} />
